...
function setupEventListeners() {
  ...
  document
    ...
    .addEventListener("click", clearAllBrainstem);
}
...
function loadBrainstem() {
  chrome.storage.local.get(["savedBrainstem"], (result) => {
    allBrainstem = Array.isArray(result.savedBrainstem)
      ? result.savedBrainstem
      : [];
...
function renderBrainstem() {
  ...
  // Render note cards
  brainstemsList.innerHTML = filteredBrainstem
  ...
  document.querySelectorAll(".add-note-btn").forEach((btn) => {
    btn.addEventListener("click", () => addNote(btn.dataset.id));
  });
}
...
function createNoteCard(note) {
  ...
  let contentHTML = `<span style="font-weight: 500;">${escapeHtml(
    note.content
  )}</span>`;
  ...
  if (note.type === "link") {
    ...
    contentHTML = `<img src="${escapeHtml(
      note.content
    )}" alt="Saved image" style="max-width: 100%; border-radius: 4px;">`;
  }

  const urlHTML =
    note.pageUrl && note.type === "text"
      ? `<a href="${escapeHtml(
          note.pageUrl
        )}" target="_blank" rel="noopener noreferrer" class="note-url">${escapeHtml(
          note.pageUrl
        )}</a>`
      : "";
    ...
    <div class="note-card">
      <div class="note-card-content">
        ...
        <div class="note-body">
          ...
          <div class="note-user-note">${note.note ? `<span class="note-user-note-label">Note:</span> ${escapeHtml(note.note)}` : ""}</div>
          <div class="note-text">
            ${contentHTML}
          </div>
          ${urlHTML}
        ...
        <div class="note-actions">
          <button class="action-btn copy-btn" data-content="${escapeHtml(
            note.content
          )}" data-type="${note.type}" data-id="${note.id}" title="Copy">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <rect x="9" y="9" width="13" height="13" rx="2" ry="2"></rect>
              <path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"></path>
            </svg>
          </button>
          <button class="action-btn add-note-btn" data-id="${
            note.id
          }" title="Add note">
...
// Add a note
function addNote(id) {
  // If already has a note, prompt to edit
  const existingNote = allBrainstem.find((note) => note.id === id);
  let message = existingNote && existingNote.note ? existingNote.note : "";
  message = window.prompt("Enter your note:", message);
  if (message) {
    const noteIndex = allBrainstem.findIndex((note) => note.id === id);
    if (noteIndex !== -1) {
      allBrainstem[noteIndex].note = message;
      chrome.storage.local.set({ savedBrainstem: allBrainstem }, () => {
        renderBrainstem();
      });
    }
